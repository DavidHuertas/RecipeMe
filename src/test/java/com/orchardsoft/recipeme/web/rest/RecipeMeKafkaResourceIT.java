...
import com.orchardsoft.recipeme.config.KafkaProperties;
...
class RecipeMeKafkaResourceIT {
    ...
    void setup() {
        ...
        RecipeMeKafkaResource kafkaResource = new RecipeMeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/recipe-me-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/recipe-me-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
